...
export const Avatar = ({ name, url, size = 'sm', className = '' }) => {
  ...
  const sizeClasses = size === 'lg' ? 'w-10 h-10 text-sm' : (size === 'xl' ? 'w-12 h-12 text-base' : 'w-6 h-6 text-[10px]');
  
  return (
    <div className={`${sizeClasses} rounded-full bg-neutral-800 flex items-center justify-center text-neutral-300 border border-neutral-700 font-medium shrink-0 ${className}`}>
